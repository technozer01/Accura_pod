...
ImageOpenCv
checkCardInFrameOrNot(Mat &refMat, Mat &src, Mat &resultMat, string filePath, bool isHologram);
...
bool ComPareTwoImage(Mat &temp, Mat &src);

bool CheckAvaibiltyofPromorydata(Mat &pclone, Mat &tclone);

float checkSubImage(Mat &pclone, Mat &tclone);
...
vector<Mat> setAllTempletes(string *textArray, int i);

map<int, Data_t> NullPointerError();
