...
-(void) Recog_MRZ:(cv::Mat&)mrzImg{
    ...
    bool bPickPhoto = YES;
    NSString* path = [[NSBundle mainBundle] pathForResource:@"key2" ofType:@"license"];
    NSLog(@"%@", path);
    ...
    if ((matImg.cols !=  720) && (matImg.cols != 0)){
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            if (!isCheckMSG){
                ...
                if (imageOpenCv.message ==  "Blur detect in document"){
        ...
        if (imageOpenCv.isSucess){
            ...
            [self performSelectorOnMainThread:@selector(recog_Successed) withObject:nil waitUntilDone:NO];
        }else{
            if(!isCheckMSG){
                dispatch_async(dispatch_get_main_queue(), ^{
                    doucumentMsg.text = @"";
