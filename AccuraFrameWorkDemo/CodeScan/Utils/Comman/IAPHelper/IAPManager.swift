...
public class IAPManager: NSObject {
    ...
    public func purchaseProduct(productId: String, completion: @escaping PurchaseProductCompletionBlock) {
        if !canMakePayments() {
            let error = NSError(domain: "inapppurchase", code: 99, userInfo: [NSLocalizedDescriptionKey: "No internet Connection"])
            completion(error)
        } else {
...
extension IAPManager { // Store file managment
    ...
    var purchasedProducts: Array<String> {
        return Array(Set(purchasedProductIds)) // Remove Duplicate values from array.
    }
    ...
    func clearInAppPurchaseTransaction() {
